...
$CurrentControl++
$SC05Data = @([PSCustomObject]@{
    GuestUserRoleId = $AuthPolicy.GuestUserRoleId
    ExpectedRole    = "Guest User"
})
...
$SC07Data = @(
    [PSCustomObject]@{
        Verification = "Manual"
        Scope        = "Management group creation permissions"
        Evidence     = "Verify whether Azure management group creation requires elevated permissions"
    }
)
...
$SC08Data = @([PSCustomObject]@{
    ConditionalAccessEnabledPolicies = $CAEnabledCount
    SecurityDefaultsEnabled          = if ($SecurityDefaults) { $SecurityDefaults.IsEnabled } else { $false }
})
...
$SC09Data = @(
    [PSCustomObject]@{
        Verification = "Manual"
        Scope        = "My Groups access"
        Evidence     = "Confirm non-admin users cannot access My Groups management features"
    }
)
...
$SC11Data = @(
    [PSCustomObject]@{
        Verification = "Manual"
        Scope        = "Membership requests in Access Panel"
        Evidence     = "Review group settings controlling owner approval for membership requests"
    }
)
...
$SC12Data = @([PSCustomObject]@{
    AllowedToCreateGroups = $AuthPolicy.DefaultUserRolePermissions.AllowedToCreateGroups
...
$SC13Data = @(
    [PSCustomObject]@{
        Verification = "Manual"
        Scope        = "Access to Entra administration portal"
        Evidence     = "Validate portal access is restricted to administrators only"
    }
)
...
$SC14Data = @(
    [PSCustomObject]@{
        Verification = "Manual"
        Scope        = "Administrative session timeout"
        Evidence     = "Validate inactivity timeout for administrative portal sessions is set to 1 hour"
    }
)
...
$SC15Data = if ($OrgBranding.Count -gt 0) {
    @(
        $OrgBranding |
        Select-Object DisplayName, SignInPageText, UsernameHintText
    )
}
else {
...
$ExpiredDomains = @(
    $Domains |
    Where-Object { $_.IsVerified -eq $false -and $_.IsDefault -eq $false } |
...
$SC18Data = @(
    [PSCustomObject]@{
        Verification = "Manual"
        Scope        = "LinkedIn account integration"
        Evidence     = "Verify users cannot connect work or school account with LinkedIn"
    }
)
